...
local docker_widget = {}
docker_widget.widget = wibox.widget {
	{
		{
			id = 'icon',
...
local function worker(user_args)
	...
	docker_widget.widget:set_icon(icon)
	...
	docker_widget.widget:buttons(
		gears.table.join(
			awful.button({}, 1, function()
				if popup.visible then
					docker_widget.widget:set_bg('#00000000')
					popup.visible = not popup.visible
				else
					docker_widget.widget:set_bg(beautiful.bg_focus)
					spawn.easy_async(string.format(LIST_CONTAINERS_CMD, executable_name, number_of_containers),
